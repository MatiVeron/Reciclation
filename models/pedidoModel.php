...
{
    $query = "SELECT pedidos.* , pedidos_estado.estado,usuarios.nombre,usuarios.apellido FROM pedidos 
            LEFT JOIN pedidos_estado ON pedidos_estado.id = pedidos.id_estado
            LEFT JOIN usuarios ON usuarios.id = pedidos.id_vecino"; // Aquí puedes agregar un JOIN si quieres obtener información relacionada como el estado o el vecino
    $stmt = $pdo->prepare($query);
    $stmt->execute();
...
function editarDetallePedido($id_detalle, $material, $cantidad, $id_estado, $tipo, $pdo)
{
    $query = "UPDATE pedidos_detalle 
              SET material = ?, cantidad = ?, tipo = ?  , id_estado = ?
              WHERE id_detalle = ?";

    $stmt = $pdo->prepare($query);
    $stmt->execute([$material, $cantidad, $tipo, $id_detalle, $id_estado]);
...
function getPedidoById($id_pedido, $pdo)
{
    $query = "SELECT * FROM pedidos LEFT JOIN pedidos_detalle ON pedidos_detalle.id_pedido = pedidos.id WHERE id_pedido = ?";
...
function actualizarPedido($id_pedido, $material, $cantidad, $tipo, $id_estado, $pdo) {
    $query = "UPDATE pedidos_detalle 
              SET material = ?, cantidad = ?, tipo = ?, id_estado = ?
              WHERE id_pedido = ?";

    $stmt = $pdo->prepare($query);
    $stmt->execute([$material, $cantidad, $tipo, $id_estado, $id_pedido]);
